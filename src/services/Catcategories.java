...
public class Catcategories implements Icategories{
    ...
    public ObservableList<Categories> affichercategorie() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                cat.setType_categories(rs.getString("type_categories"));
   ...
   @Override
    public Categories findcatById(int id_cat) {
          Categories cat = new Categories();
        String requete = "SELECT * FROM categories WHERE id_cat=?";
        try {
            ...
            while (resultat.next()) {
                ...
                cat.setType_categories(resultat.getString("type_categories"));
            }
            return cat;
    ...
    @Override
    public Categories findcatBynom(String type_categories) {
           Categories cat = new Categories();
        String requete = "SELECT * FROM categories WHERE type_categories=?";
        try {
            PreparedStatement ps = cnx.prepareStatement(requete);
            ps.setString(1, type_categories);
            ResultSet resultat = ps.executeQuery();
            while (resultat.next()) {
                cat.setId_cat(resultat.getInt(1));
                cat.setType_categories(resultat.getString(2));
            }
            return cat;
